...
export default function HeroNavigator({ stage }: { stage: number }) {
  ...
  const accentColor = activeCampaign?.theme.accent || '#C89B5A';
  ...
  return (
    ...
    >
      {/* Current slide number — large, gold */}
      <motion.span
        key={activeIndex}
      ...
      {/* Top connecting line — gold gradient */}
      <div
        ...
        {campaigns.map((c, i) => {
          ...
          return (
            <button
              ...
              className="group relative flex items-center justify-center w-8 h-6 focus:outline-none focus-visible:ring-1 focus-visible:ring-white/30 rounded"
